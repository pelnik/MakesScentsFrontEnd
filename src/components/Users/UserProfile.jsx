        ...
        {orders.map((order, idx) => {
          return(
            <div className='OrderCard' key={'order idx:' + idx}>
              <h1 className='OrderNumber'>Order <span className='Info'>#{order.id}</span></h1>
              {order.items.map((item, idx) => {
                return(
                  <div key={'item idx:' + idx}>
                    <h2 className='OrderInfo'>Item: <span className='Info'>{item.product_name}</span></h2>
                    <h3 className='ItemInfo'>Size: <span className='Info'>{item.product_size}</span>    Quantity: <span className='Info'>{item.quantity}</span>    Color: <span className='Info'>{item.product_color}</span>    Fragrance: <span className='Info'>{item.product_fragrance}</span></h3>
                  </div>
                )
              })}
              <h2 className='OrderInfo'>Order Status: <span className='Info'>{order.status}</span></h2>
              </div>
          )
        })}
      </div>
